...
public class DcBqcBingoCheck implements DcBingoCheck {
	/*
	...
	public void addCheck(BingoCheck otherCheck) {
		...
		DcBqcBingoCheck check = (DcBqcBingoCheck) otherCheck;
		if (!check.isOpenAble) {
	...
	public static int getResultByBF(String halfScore, String wholeScore, String concede) {
		if(halfScore==null || wholeScore==null){
			return -1;
		}
		
		// 比赛取消
		if (halfScore.equals("*") && wholeScore.equals("*")) {
			return 9999;
		}
		int homeScore = 0;
		int awaryScore = 0;
		int halfhomeScore = 0;
		int halfawaryScore = 0;
		String[] socreArea = wholeScore.split(":");
		homeScore = Integer.valueOf(socreArea[0]);
		awaryScore = Integer.valueOf(socreArea[1]);

		String[] halfsocreArea = halfScore.split(":");
		halfhomeScore = Integer.valueOf(halfsocreArea[0]);
		halfawaryScore = Integer.valueOf(halfsocreArea[1]);

		if (halfhomeScore > halfawaryScore) {
			if (homeScore > awaryScore)// 胜-胜
				return 0;
			else if (homeScore == awaryScore)// 胜-负
				return 1;
			else
				return 2;
		} else if (halfhomeScore == halfawaryScore) {// 平-胜
			if (homeScore > awaryScore)
				return 3;
			else if (homeScore == awaryScore)// 平-平
				return 4;
			else
				return 5;// 平-负
		} else {
			if (homeScore > awaryScore)// 负-胜
				return 6;
			else if (homeScore == awaryScore)// 负-平
				return 7;
			else
				return 8;// 负-负
		}
