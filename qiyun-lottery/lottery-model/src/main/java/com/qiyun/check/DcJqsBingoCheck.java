...
public class DcJqsBingoCheck implements DcBingoCheck {
	/*
	...
	public void addCheck(BingoCheck otherCheck) {
		...
		DcJqsBingoCheck check = (DcJqsBingoCheck) otherCheck;
		if (!check.isOpenAble) {
	...
	public void execute(Ticket ticket, HashMap<String, String> openResultMap) {
		...
		if (ticket.getPlayType().getValue() == PlayType.DC_1.getValue()) {
			...
			for (int i = 0; i < matchAreas.length; i++) {
				int matchNo = Integer.valueOf(getBeforeGuoHao(matchAreas[i]));
				// 如果还有比赛是没有结果的，不作开奖处理
	...
	public static int getResultByBF(String halfScore, String wholeScore) {
		// 比赛取消
		...
		if (homeScore + awaryScore + 1 > 7)
			return 7;
		else
			return homeScore + awaryScore;
	}
	...
	private String getBeforeGuoHao(String guoHaoString) {
		return guoHaoString.substring(0, guoHaoString.indexOf('('));
	}
	...
	public static void main(String[] ar) {
		DcJqsBingoCheck check = new DcJqsBingoCheck();
		...
		openResultMap.put("halfScore1", "4:0");
		openResultMap.put("halfScore2", "3:1");
		openResultMap.put("halfScore3", "0:1");

		openResultMap.put("wholeScore1", "4:0");
		...
		check.execute("101:1(4)", PlayType.DC_1, 1, openResultMap);
