...
public class DcSfggBingoCheck implements DcBingoCheck {
	/*
	...
	public void addCheck(BingoCheck otherCheck) {
		...
		DcSfggBingoCheck check = (DcSfggBingoCheck) otherCheck;
		if (!check.isOpenAble) {
	...
	public void execute(Ticket ticket, HashMap<String, String> openResultMap) {
		...
		if (passCount == matchAreas.length) {
			...
			if(term>=140510 && term <=140603){
				//加奖5%,但不产生税金 公式=含税奖金 / 65% * 5%;
				double add= 2 * spSum *0.05;
				_post=_post+add;
				_pre=_pre+add;
				//
			}else if(term >= 140604 && term <=140703){
				//加奖6%,但不产生税金 公式=含税奖金 / 65% * 6%;
				double add= 2 * spSum *0.06;
				_post=_post+add;
				_pre=_pre+add;
			}
			//加奖结束
