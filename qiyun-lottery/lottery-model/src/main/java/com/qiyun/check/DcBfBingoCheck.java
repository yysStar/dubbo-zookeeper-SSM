...
public class DcBfBingoCheck implements DcBingoCheck {
	/*
	...
	public void addCheck(BingoCheck otherCheck) {
		...
		DcBfBingoCheck check = (DcBfBingoCheck) otherCheck;
		if (!check.isOpenAble) {
	...
	public void execute(Ticket ticket, HashMap<String, String> openResultMap) {
		...
		if (ticket.getPlayType().getValue() == PlayType.DC_1.getValue()) {
			...
			for (int i = 0; i < matchAreas.length; i++) {
				...
				// 如果还有比赛是没有结果的，不作开奖处理
				if (openResultMap.get("spResult" + matchNo)==null) {
					isOpenAble = false;
					return;
				}
				...
				for (int j = 0; j < options.length; j++) {
					...
					if (Integer.valueOf(options[j]) == matchResult || matchResult == 9999) {
						...
						// 中奖了
						double danguanbingo = 2 * spResultHashMap.get("spResult" + matchNo) * 0.65;
						//加奖开始
	...
	public static int getResultByBF(String wholeScore) {
		...
		if (Integer.parseInt(socreArea[0]) > Integer.parseInt(socreArea[1])) {
			if (wholeScore.equals("1:0"))
				return 1;
			else if (wholeScore.equals("2:0"))
				return 2;
			else if (wholeScore.equals("2:1"))
				return 3;
			else if (wholeScore.equals("3:0"))
				return 4;
			else if (wholeScore.equals("3:1"))
				return 5;
			else if (wholeScore.equals("3:2"))
				return 6;
			else if (wholeScore.equals("4:0"))
				return 7;
			else if (wholeScore.equals("4:1"))
				return 8;
			else if (wholeScore.equals("4:2"))
				return 9;
			else
				return 0;
		} else if (Integer.parseInt(socreArea[0]) == Integer.parseInt(socreArea[1])) {
			if (wholeScore.equals("0:0"))
				return 11;
			else if (wholeScore.equals("1:1"))
				return 12;
			else if (wholeScore.equals("2:2"))
				return 13;
			else if (wholeScore.equals("3:3"))
				return 14;
			else
				return 10;

		} else {
			if (wholeScore.equals("0:1"))
				return 16;
			else if (wholeScore.equals("0:2"))
				return 17;
			else if (wholeScore.equals("1:2"))
				return 18;
			else if (wholeScore.equals("0:3"))
				return 19;
			else if (wholeScore.equals("1:3"))
				return 20;
			else if (wholeScore.equals("2:3"))
				return 21;
			else if (wholeScore.equals("0:4"))
				return 22;
			else if (wholeScore.equals("1:4"))
				return 23;
			else if (wholeScore.equals("2:4"))
				return 24;
			else
				return 15;
		}
	...
	/*
	 * private static int getValue(String wholeScore){ String [] socreArea =
	 * wholeScore.split(":");
	 * if(Integer.parseInt(socreArea[0])>Integer.parseInt(socreArea[0])){
	 * if(wholeScore.equals("1:0")) return 2; else if(wholeScore.equals("2:0"))
	 * return 3; else if(wholeScore.equals("2:1")) return 4; else
	 * if(wholeScore.equals("3:0")) return 5; else if(wholeScore.equals("3:1"))
	 * return 6; else if(wholeScore.equals("3:2")) return 7; else
	 * if(wholeScore.equals("4:0")) return 8; else if(wholeScore.equals("4:1"))
	 * return 9; else if(wholeScore.equals("4:2")) return 10; else return 1; }
	 * else if(Integer.parseInt(socreArea[0])==Integer.parseInt(socreArea[0])){
	 * if(wholeScore.equals("0:0")) return 12; else if(wholeScore.equals("1:1"))
	 * return 13; else if(wholeScore.equals("2:2")) return 14; else
	 * if(wholeScore.equals("3:3")) return 15; else return 11; } else{
	 * if(wholeScore.equals("0:1")) return 17; else if(wholeScore.equals("0:2"))
	 * return 18; else if(wholeScore.equals("1:2")) return 19; else
	 * if(wholeScore.equals("0:3")) return 20; else if(wholeScore.equals("1:3"))
	 * return 21; else if(wholeScore.equals("2:3")) return 22; else
	 * if(wholeScore.equals("0:4")) return 23; else if(wholeScore.equals("1:4"))
	 * return 24; else if(wholeScore.equals("2:4")) return 25; else return 16; } }
	 */
	// 拿到括号里面的字符串
	...
	public static void main(String[] ar) {
		DcBfBingoCheck check = new DcBfBingoCheck();

		HashMap<String, String> openResultMap = new HashMap<String, String>();
		openResultMap.put("spResult1", "5");
		openResultMap.put("spResult2", "1");
		openResultMap.put("spResult3", "10");

		openResultMap.put("halfScore1", "1:1");
		openResultMap.put("halfScore2", "*");
		openResultMap.put("halfScore3", "0:1");

		openResultMap.put("wholeScore1", "4:1");
		openResultMap.put("wholeScore2", "*");
		...
		openResultMap.put("result", "1(1),2(*),3(0)");
		openResultMap.put("term", "100518");

		check.execute("102:1(4);2(8)", PlayType.DC_1, 1, openResultMap);

		LogUtil.info(check.getBingoContent());

		// 2.63,2.29,2.5,2.72,2.1,2.06,3.4,3.52,1.96,3.05,1.83

		/*
		 * double [] f = new
		 * double[]{2.63f,2.29f,2.5f,2.72f,2.1f,2.06f,3.4f,3.52f,1.96f,3.05f,1.83f};
		 * DecimalFormat DF = new DecimalFormat("#.00");
		 * 
		 * double result = 0f; for(int i=0;i<f.length;i++){ //result +=
		 * Double.valueOf(DF.format(f[i]*2*0.65f)); result +=
		 * DcSfpBingoCheck.round(f[i]*2*0.65f,2); }
		 */
	}
