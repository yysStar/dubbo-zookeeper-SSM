...
public class DcSfpBingoCheck implements DcBingoCheck {
	/*
	...
	public void addCheck(BingoCheck otherCheck) {
		...
		DcSfpBingoCheck check = (DcSfpBingoCheck) otherCheck;
		if (!check.isOpenAble) {
		 ...
		 * 
		 * String [] results = resultSb.toString().split("\\,"); for(int i=0;i<results.length;i++){
		 * hashset.add(results[i]); }
		 * 
		 * results = check.getResultSb().toString().split("\\,"); for(int i=0;i<results.length;i++){
		 * hashset.add(results[i]); }
		 * 
		 * Iterator<String> it = hashset.iterator(); resultSb = new
		 * StringBuffer(); while(it.hasNext()){
		 * resultSb.append(it.next()).append(","); }
		...
		buildBingoContent();
		// this.bingoPosttaxTotal +=
		// BigDecimal.valueOf(Double.valueOf(DF.format(otherCheck.getBingoPosttaxTotal()))).doubleValue();
		// this.bingoPretaxTotal +=
		// BigDecimal.valueOf(Double.valueOf(DF.format(otherCheck.getBingoPretaxTotal()))).doubleValue();

		this.bingoPosttaxTotal += Double.valueOf(DF.format(otherCheck.getBingoPosttaxTotal())).doubleValue();
		this.bingoPretaxTotal += Double.valueOf(DF.format(otherCheck.getBingoPretaxTotal())).doubleValue();
		...
		if (ticket.getPlayType().getValue() == PlayType.DC_1.getValue()) {
			...
			for (int i = 0; i < matchAreas.length; i++) {
				...
				for (int j = 0; j < options.length; j++) {
					// int matchResult = getResultByBF(openResultMap, matchNo);
					int matchResult = getResultByBF(openResultMap.get("halfScore" + matchNo), openResultMap.get("wholeScore" + matchNo),
							openResultMap.get("concede" + matchNo));
					// 延期即9999，也算中
	...
	// 返回3胜,1平,0负
	public static int getResultByBF(String halfScore, String wholeScore, String concede) {
		// 比赛取消
		...
		if (homeScore + Double.valueOf(concede) == awaryScore) {
			return 1;
		} else if (homeScore + Double.valueOf(concede) > awaryScore) {
			return 3;
		} else {
			return 0;
		}
	...
	public static void main(String[] ar) {
		DcSfpBingoCheck check = new DcSfpBingoCheck();
		...
		openResultMap.put("halfScore1", "*");
		openResultMap.put("halfScore2", "3:1");
		...
		check.execute("101:1(1)", PlayType.DC_1, 1, openResultMap);
