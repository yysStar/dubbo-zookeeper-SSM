			...
			while(it.hasNext()){
				...
				if(item.containsKey("ttg")){
					...
					if("Selling".equals(pStatus)) {
						...
						award.setS0Award(jsonObj.getDouble("s0"));
						award.setS1Award(jsonObj.getDouble("s1"));
						award.setS2Award(jsonObj.getDouble("s2"));
						award.setS3Award(jsonObj.getDouble("s3"));
						award.setS4Award(jsonObj.getDouble("s4"));
						award.setS5Award(jsonObj.getDouble("s5"));
						award.setS6Award(jsonObj.getDouble("s6"));
						award.setS7Award(jsonObj.getDouble("s7"));
