...
public class QxcTermBuilder extends AbstractLotteryTermBuider {

    private static HashMap<Integer, Integer> nextOpenDates = null;

    static {
        nextOpenDates = new HashMap<Integer, Integer>();
        nextOpenDates.put(1, 2);
        nextOpenDates.put(3, 3);
        nextOpenDates.put(6, 2);
    }
    ...
    public List<LotteryTerm2> build(String oldTerm, Date oldOpenDateTime, int buildCount) {
        ...
        for (int i = 1; i <= buildCount; i++) {
            ...
            LotteryTerm2 lotteryTerm = createLotteryTerm(LotteryType.QXC,oldTerm,startDateTime,endDateTime,terminalEndDateTime,openDateTime);
