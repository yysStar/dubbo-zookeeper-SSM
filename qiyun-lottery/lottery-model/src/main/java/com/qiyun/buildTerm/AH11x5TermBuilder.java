...
public class AH11x5TermBuilder extends AbstractLotteryTermBuider {
    ...
    public List<LotteryTerm2> build(String oldTerm, Date oldOpenDateTime, int buildCount) {
        ...
        for (int i = 1; i <= buildCount; i++) {
            ...
            if (Integer.parseInt(oldTerm.substring(oldTerm.length() - 2, oldTerm.length())) < 81) {
                ...
                calendar.set(Calendar.MINUTE, 38); // 08:38:30 我们提前30s截止
                // 接口是提前1分钟截止
            ...
            LotteryTerm2 lotteryTerm = createLotteryTerm(LotteryType.AH11X5,oldTerm,startDateTime,endDateTime,terminalEndDateTime,openDateTime);
