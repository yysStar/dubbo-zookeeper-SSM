...
public class HEBK3TermBuilder extends AbstractLotteryTermBuider {
    ...
    public List<LotteryTerm2> build(String oldTerm, Date oldOpenDateTime, int buildCount) {
        ...
        for (int i = 1; i <= buildCount; i++) {
            ...
            // 开奖期次
            if (Integer.parseInt(oldTerm.substring(oldTerm.length() - 2, oldTerm.length())) < 81) {
                // 关闭销售时间
                ...
                String sterm = "";
                if ((Integer.parseInt(oldTerm.substring(6, 9)) + 1) > 9) {
                    sterm = "0" + (Integer.parseInt(oldTerm.substring(6, 9)) + 1);
                } else {
                    sterm = "00" + (Integer.parseInt(oldTerm.substring(6, 9)) + 1);
                }
                ...
                calendar.set(Calendar.MINUTE, 36); // 08:36:30 我们提前30s截止
                calendar.set(Calendar.SECOND, 30);

                // 接口是提前1分钟截止 (官方08:38:00截止)
                endDateTime = calendar.getTime();
            ...
            LotteryTerm2 lotteryTerm = createLotteryTerm(LotteryType.HEB_K3,oldTerm,startDateTime,endDateTime,terminalEndDateTime,openDateTime);
