...
public class ZJ11x5TermBuilder extends AbstractLotteryTermBuider {
    ...
    public List<LotteryTerm2> build(String oldTerm, Date oldOpenDateTime, int buildCount) {
        ...
        for (int i = 1; i <= buildCount; i++) {
            ...
            // 开奖期次
            if (Integer.parseInt(oldTerm.substring(oldTerm.length() - 2, oldTerm.length())) < 80) {
                // 关闭销售时间
                ...
                calendar.set(Calendar.MINUTE, 48); // 08:48:30 我们提前30s截止
                // 接口是提前1分钟截止
            ...
            LotteryTerm2 lotteryTerm = createLotteryTerm(LotteryType.ZJ11X5,oldTerm,startDateTime,endDateTime,terminalEndDateTime,openDateTime);
