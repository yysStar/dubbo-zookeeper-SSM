...
public class HEN11x5TermBuilder extends AbstractLotteryTermBuider {
    ...
    public List<LotteryTerm2> build(String oldTerm, Date oldOpenDateTime, int buildCount) {
        ...
        for (int i = 1; i <= buildCount; i++) {
            ...
            if (Integer.parseInt(oldTerm.substring(oldTerm.length() - 2, oldTerm.length())) < 72) {
                ...
                calendar.set(Calendar.MINUTE, 8); // 10:08:30 我们提前30s截止
                // 接口是提前1分钟截止
            ...
            LotteryTerm2 lotteryTerm = createLotteryTerm(LotteryType.HEN11X5,oldTerm,startDateTime,endDateTime,terminalEndDateTime,openDateTime);
