...
public class AHK3TermBuider extends AbstractLotteryTermBuider {
    int terminalDelay = 115; // s
    int openDelay = 15; // s
    ...
    public List<LotteryTerm2> build(String oldTerm, Date oldOpenDateTime, int buildCount) {
        ...
        for (int i = 1; i <= buildCount; i++) {
            ...
            // 开奖期次
            if (Integer.parseInt(oldTerm.substring(6, 9)) < 80) {
                // 关闭销售时间
                ...
                calendar.set(Calendar.MINUTE, 48);// 08:48 我们提前30s截止
                calendar.set(Calendar.SECOND, 5);

                // 接口是提前1分25秒截止 (官方8:50截止)
                endDateTime = calendar.getTime();
            ...
            LotteryTerm2 lotteryTerm = createLotteryTerm(LotteryType.AH_K3, oldTerm, startDateTime, endDateTime, terminalEndDateTime, openDateTime);
