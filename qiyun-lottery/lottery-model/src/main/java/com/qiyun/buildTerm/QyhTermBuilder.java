...
public class QyhTermBuilder extends AbstractLotteryTermBuider {
    int terminalDelay = 180; // s
    int openDelay = 60; // s
    int startSaleDelay = terminalDelay + openDelay; // 开奖时间前x分钟开售下期
    int stopSaleDelay = 15 * 60; // 销售期 15分钟
    ...
    public List<LotteryTerm2> build(String oldTerm, Date oldOpenDateTime, int buildCount) {
        ...
        for (int i = 1; i <= buildCount; i++) {
            ...
            // 开奖期次
            if (Integer.parseInt(oldTerm.substring(oldTerm.length() - 2, oldTerm.length())) < 52) {
                // 关闭销售时间
                ...
                if ((Integer.parseInt(oldTerm.substring(oldTerm.length() - 2, oldTerm.length())) + 1) > 9) {
                    sterm = "K0" + (Integer.parseInt(oldTerm.substring(oldTerm.length() - 2, oldTerm.length())) + 1);
                } else {
                    sterm = "K00" + (Integer.parseInt(oldTerm.substring(oldTerm.length() - 2, oldTerm.length())) + 1);
                }
                ...
                calendar.set(Calendar.MINUTE, 12); // 09:12:00 我们提前1分钟截止
                // 接口是提前2分钟截止 (官方9:15截止)
            ...
            LotteryTerm2 lotteryTerm = createLotteryTerm(LotteryType.SDQYH,oldTerm,startDateTime,endDateTime,terminalEndDateTime,openDateTime);
