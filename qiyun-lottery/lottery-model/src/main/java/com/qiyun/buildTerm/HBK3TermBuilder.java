...
public class HBK3TermBuilder extends AbstractLotteryTermBuider {
    ...
    public List<LotteryTerm2> build(String oldTerm, Date oldOpenDateTime, int buildCount) {
        ...
        for (int i = 1; i <= buildCount; i++) {
            ...
            // 开奖期次
            if (Integer.parseInt(oldTerm.substring(6, 9)) < 78) {
                // 关闭销售时间
                ...
                calendar.set(Calendar.MINUTE, 8);// 09:8:00 我们提前30s截止
                calendar.set(Calendar.SECOND, 0);

                // 接口是提前1分钟30截止 (官方9:10截止)
                endDateTime = calendar.getTime();
            ...
            LotteryTerm2 lotteryTerm = createLotteryTerm(LotteryType.HB_K3,oldTerm,startDateTime,endDateTime,terminalEndDateTime,openDateTime);
