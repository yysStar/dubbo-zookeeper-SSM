...
public class SHK3TermBuilder extends AbstractLotteryTermBuider {
    ...
    public List<LotteryTerm2> build(String oldTerm, Date oldOpenDateTime, int buildCount) {
        ...
        for (int i = 1; i <= buildCount; i++) {
            ...
            // 开奖期次
            if (Integer.parseInt(oldTerm.substring(oldTerm.length() - 2, oldTerm.length())) < 82) {
                // 关闭销售时间
                ...
                calendar.set(Calendar.MINUTE, 8); // 09:08:30 我们提前30s截止
            ...
            LotteryTerm2 lotteryTerm = createLotteryTerm(LotteryType.SH_K3,oldTerm,startDateTime,endDateTime,terminalEndDateTime,openDateTime);
