...
public class GSK3TermBuilder extends AbstractLotteryTermBuider {
    ...
    public List<LotteryTerm2> build(String oldTerm, Date oldOpenDateTime, int buildCount) {
        ...
        for (int i = 1; i <= buildCount; i++) {
            ...
            if (Integer.parseInt(oldTerm.substring(oldTerm.length() - 2, oldTerm.length())) < 72) {
                ...
                calendar.set(Calendar.MINUTE, 16); // 10:16:30 我们提前30s截止
                calendar.set(Calendar.SECOND, 30);

                // 接口是提前1分钟截止 (官方10:20:00截止)
                endDateTime = calendar.getTime();
            ...
            LotteryTerm2 lotteryTerm = createLotteryTerm(LotteryType.GS_K3,oldTerm,startDateTime,endDateTime,terminalEndDateTime,openDateTime);
