...
public class JL11x5TermBuilder extends AbstractLotteryTermBuider {
    ...
    public List<LotteryTerm2> build(String oldTerm, Date oldOpenDateTime, int buildCount) {
        ...
        for (int i = 1; i <= buildCount; i++) {
            ...
            // 开奖期次
            if (Integer.parseInt(oldTerm.substring(oldTerm.length() - 2, oldTerm.length())) < 79) {
                // 关闭销售时间
            ...
            LotteryTerm2 lotteryTerm = createLotteryTerm(LotteryType.JL11X5,oldTerm,startDateTime,endDateTime,terminalEndDateTime,openDateTime);
