...
import com.qiyun.grabber.AbstractLotteryTerm;
...
/**
 * LotteryTerm generated by MyEclipse Persistence Tools
 */
public class LotteryTerm extends AbstractLotteryTerm implements java.io.Serializable {
	...
	public LotteryTerm(Integer id, LotteryType lotteryType, String term, Date openDateTime, Date startDateTime, Date endDateTime,
		...
		super(id, lotteryType, term, openDateTime, startDateTime, endDateTime, terminalEndDateTime, isAble, isCurrentTerm, status, result,
				resultDetail, awardPool, testMachineCode);
	}

	public List getPrizeBeanList() {
		String results = this.getResultDetail();
		if (results == null || "".equals(results)) {
			return new ArrayList();
		}
		return PrizeCommon.buildToList(super.getLotteryType().getValue(), results);
	}
	...
	public List getNewPrizeBeanList() {
		...
		for (int i = 0; i < prizePiece.length; i++) {
			...
			bean.setLotteryType(super.getLotteryType().getValue());
			bean.setPrizeName(PrizeListDef.get(super.getLotteryType().getValue()).get(areas[0]));
	...
	public List<String> getPrizeList() {
		...
		String totalContent = super.getResultDetail();
	...
	public Date getHmEndDateTime() {
		if (this.hmEndDateTime == null)
			return new Date(super.getEndDateTime().getTime() - 15 * 60 * 1000);
		else
	...
	public String getZstUseResult() {
		...
		if(getLotteryType() != null && getLotteryType().getValue() == LotteryType.SSQ.getValue()) {
	...
	public String getCodeShape() {
		try {
			if (getLotteryType().getValue() == LotteryType.K3.getValue() ||
				getLotteryType().getValue() == LotteryType.JL_K3.getValue() ||
				getLotteryType().getValue() == LotteryType.HB_K3.getValue() ||
				getLotteryType().getValue() == LotteryType.AH_K3.getValue() ||
				getLotteryType().getValue() == LotteryType.JX_K3.getValue() ||
				getLotteryType().getValue() == LotteryType.HB_K3.getValue()) {// 暂快3有号码形态
	...
	public String getResultShape() {
		if (getLotteryType() == LotteryType.POKER3) {
			...
			return sb.toString();
		} else if(getLotteryType()== LotteryType.SSQ){
			String result=getResult();
			...
			return result;
		}else if(getLotteryType()== LotteryType.DLT){
			String result=getResult();
			if(!StringTools.isNullOrBlank(result)){
				String[] arr=result.split(",");
				StringBuilder red=new StringBuilder("");
	...
	public boolean termIsClose(TradeType tradeType){
		if(this.getIsAble().getValue() == 1 && tradeType == TradeType.TRADE_BALANCE){
			return true;
		}else if(this.getLotMgrIsAble().getValue() == 1 && tradeType == TradeType.SCORE_BET){
			return true;
		}
		return false;
	}
