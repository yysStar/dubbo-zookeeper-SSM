...
public class XmlUtils {
    ...
    public static String beansToXmlStr(List daiFus){
        if (daiFus==null || daiFus.size()<=0){
            return null;
        }
        StringBuffer sb = new StringBuffer();
        for (Object daiFu : daiFus) {
            sb.append("<item>");
            Field[] fields=daiFu.getClass().getDeclaredFields();
            for (Field field : fields) {
                ...
                Method method = null;
                try {
                    method = daiFu.getClass().getMethod(getter, new Class[] {});
                    Object value = method.invoke(daiFu, new Object[] {});
                    sb.append("<"+key+">").append(value==null?"":value.toString()).append("</"+key+">");
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
